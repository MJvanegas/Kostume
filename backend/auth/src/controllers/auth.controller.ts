...
import { MongoError } from '../interfaces/mongo-error.interface';
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (mongoError.code === 11000)
        return res
          .status(409)
          .json({ status: 'error', error: mongoError.message });
      res.status(400).json({ error: mongoError.message });
    } else {
...
router.post('/verify', validateApiKey, (req, res) => {
  try {
